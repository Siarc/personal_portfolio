...
function LiveCodingTerminal({ currentTheme }: { currentTheme: Theme }) {
  ...
  const codingScenarios = [
    ...
    {
      command: "git commit -m 'feat: particle animation'",
      steps: [
        "📝 Staging changes...",
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentScenario((prev) => (prev + 1) % codingScenarios.length)
    }, 8000) // Change scenario every 8 seconds

    return () => clearInterval(interval)
  }, [])

  const scenario = codingScenarios[currentScenario]

  return (
    <div className="p-4 font-mono text-sm overflow-hidden" style={{ height: '170px' }}>
      <motion.div
        key={currentScenario}
        className="space-y-1"
        initial={{ opacity: 0 }}
      ...
      >
        ...
        {scenario.steps.map((step, index) => (
          <motion.div
            key={index}
            style={{
              color: index === scenario.steps.length - 1
                ? currentTheme.colors.primary
                : currentTheme.colors.secondary
            }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 2 + index * 1.2, duration: 0.5 }}
          >
            {step}
          </motion.div>
        ))}
        ...
        >
          ...
          <motion.span
            ...
            transition={{ duration: 1, repeat: Infinity, delay: 7 }}
          >
            |
          </motion.span>
        </motion.div>
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-8 mb-12">
          ...
          <div className="flex flex-col items-center">
            ...
            >
              ...
              {/* Terminal Content */}
              <LiveCodingTerminal currentTheme={currentTheme} />
            </div>
